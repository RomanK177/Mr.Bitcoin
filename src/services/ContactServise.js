...
function query(filterBy) {
  ...
  if (filterBy) {
    ...
    contactsToReturn = gContacts.filter(
      (contact) =>
        contact.name
          .toLocaleLowerCase()
          .includes(filterBy.filterBy.toLocaleLowerCase()) ||
        contact.email
          .toLocaleLowerCase()
          .includes(filterBy.filterBy.toLocaleLowerCase()) ||
        contact.phone.includes(filterBy.filterBy)
    );
  }
  // if (filterBy) {
  //   var { name, email, phone } = filterBy;
  ...
  //       contact.email.toLocaleLowerCase().includes(email.toLocaleLowerCase()) &&
...
function remove(id) {
  const idx = gContacts.findIndex((robot) => robot._id === id);
